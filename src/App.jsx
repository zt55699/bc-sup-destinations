...
function App() {
    ...
    const destinations = [
        ...
        { id: 25, name: 'Belcarra Park', coords: [49.3122826, -122.9274902], tags: ['海水', '公园', '满地宝', '家庭'], difficulty: 'beginner', difficultyColor: '#22c55e', description: 'Belcarra Regional Park 提供了一个美丽的野餐区和一个码头，使其成为探索 Indian Arm 平静水域的便捷下水点。这里是各级别桨手的理想之地，拥有迷人的景色和附近的步道。', difficultyInfo: '● 初级 - 公园环境，码头设施，适合各级别', imageUrls: ['https://theoutdoorfamilyblog.wordpress.com/wp-content/uploads/2020/09/img_1616.jpg?w=1024', 'https://campbellriverwhalewatching.com/wp-content/uploads/2020/06/Copy-of-desolation-sound-068-kayak-trips-copy-1024x639.jpg', 'https://worldadventurists.com/wp-content/uploads/2020/08/jugisland2020_029-780x520.jpg'] },
    ...
    function showDetailCard(destination) {
        ...
        if (detailCard) {
            ...
            // zoom 8 = 1000ms, zoom 15+ = 0ms, linear interpolation
            const delay = Math.max(0, Math.min(1000, 1000 - (currentZoom - 8) * 143));
            ...
            if (delay > 0) {
                ...
                // No delay for closest zoom levels
                detailCard.classList.remove('translate-y-full');
            }
        }
