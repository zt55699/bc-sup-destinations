...
function App() {
    ...
    const destinations = [
        ...
        { id: 24, name: 'Sproat Lake', coords: [49.2370, -125.0000], tags: ['淡水', '湖泊', '温哥华岛', '热门'], difficulty: 'beginner', difficultyColor: '#22c55e', description: 'Sproat Lake 是温哥华岛上一个大型的暖水湖，非常受家庭欢迎。这里不仅有广阔的水域可供探索，还有著名的史前岩画（Petroglyphs）可供观赏。', difficultyInfo: '● 初级 - 暖水湖，家庭友好，设施完善', imageUrl: 'https://images.unsplash.com/photo-1440779696743-62a681ddc68f?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&h=400&q=80' },
        ...
        { id: 32, name: 'Sandy Beach', coords: [49.3780, -123.3350], tags: ['海水', '沙滩', '宝云岛', '家庭'], difficulty: 'beginner', difficultyColor: '#22c55e', description: 'Sandy Beach 位于宝云岛的东侧，靠近 Snug Cove 码头，是一个方便到达的沙滩。这里的水域相对平静，非常适合家庭和初学者进行休闲划桨。', difficultyInfo: '● 初级 - 平静沙滩，家庭友好，方便到达', imageUrl: 'https://placehold.co/800x400/FBBF24/FFFFFF?text=Sandy+Beach' },
        { id: 33, name: 'Tunstall Bay', coords: [49.3800, -123.3850], tags: ['海水', '日落', '宝云岛', '风景'], difficulty: 'intermediate', difficultyColor: '#eab308', description: 'Tunstall Bay 位于宝云岛的西海岸，是观赏壮丽日落的绝佳地点。这里的海滩更具野性，水域可能更具挑战性，适合有一定经验的桨手。', difficultyInfo: '■ 中级 - 野性海滩，更具挑战性', imageUrl: 'https://placehold.co/800x400/F472B6/FFFFFF?text=Tunstall+Bay' },
    ];
    ...
    function selectDestination(id) {
        ...
        if (mapInstanceRef.current && destination) {
            mapInstanceRef.current.flyTo(destination.coords, 13, {
                animate: true,
                duration: 1.5
            });
        }
    ...
    function showDetailCard(destination) {
        ...
        if (tagsContainer) {
            tagsContainer.innerHTML = destination.tags.map(tag => 
                `<span class="text-sm text-amber-300 bg-amber-500/20 px-3 py-1 rounded-full">${tag}</span>`
            ).join('');
        }
    ...
    return (
        <div className="bg-gray-900 overflow-hidden h-screen">
            ...
            {/* 底部详情卡片 */}
            <div id="detail-card" className="fixed bottom-0 left-0 right-0 z-20 p-4 transform translate-y-full transition-transform duration-500 ease-in-out">
                <div className="max-w-4xl mx-auto bg-black/30 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl overflow-hidden">
                    <div className="relative">
                        ...
                        <button id="close-detail-card" onClick={hideDetailCard} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center bg-black/40 backdrop-blur-md rounded-full text-white hover:bg-white/30 transition-colors">
                    ...
                    <div className="p-5 md:p-6">
                        ...
                        <div id="detail-difficulty-info" className="mt-2 text-sm text-amber-300 bg-amber-500/20 px-3 py-2 rounded-lg">
                            {/* 难度说明将通过 JS 动态插入 */}
                        </div>
                        ...
                        <div className="mt-6">
                            <a id="detail-gmaps-link" href="#" target="_blank" rel="noopener noreferrer" className="inline-block w-full text-center bg-sky-500 text-white font-semibold py-3 px-6 rounded-xl hover:bg-sky-600 transition-colors duration-300">
                                在谷歌地图上查看
                            </a>
                        </div>
                    </div>
